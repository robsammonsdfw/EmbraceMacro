...
export const PantryChefView: React.FC<PantryChefViewProps> = ({ savedMeals, onSaveMeal }) => {
    ...
    const handleSaveRecipe = (recipe: Recipe) => {
        ...
        // This is a bit of a hack since Recipe != SavedMeal exactly, but SavedMeal supports recipe data structure
        const mealToSave = {
            ...
            recipe: recipe,
            source: 'medical-ai' as const // or create a 'pantry-chef' source type
        };
        onSaveMeal(mealToSave);
        alert("Recipe saved to Library!");
    };

    // Filter SavedMeals to show only those that have recipe data (Saved Recipes)
    const savedRecipes = savedMeals.filter(m => m.recipe);
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-full flex flex-col">
                    ...
                    {savedRecipes.length > 0 ? (
                        ...
                        <div className="flex-grow flex flex-col items-center justify-center text-slate-400 py-10">
                            ...
                            <p className="text-xs font-bold uppercase">No saved recipes</p>
                        </div>
                    )}
                </div>
